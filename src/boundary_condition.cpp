  ...
  : PeriodicBC(i_surf, j_surf)
{
  ...
  double d1;
  // Note: time = speed = 0.0, moving boundary surface is not supported
  double e1 = surf1.evaluate(origin, 0.0);
  if (e1 > FP_COINCIDENT) {
    d1 = -surf1.distance(origin, -u, 0.0, 0.0, false);
  ...
  double d2;
  double e2 = surf2.evaluate(origin, 0.0);
  if (e2 > FP_COINCIDENT) {
    d2 = -surf2.distance(origin, -u, 0.0, 0.0, false);
  ...
  : PeriodicBC(i_surf, j_surf)
{
  ...
  // time = 0.0, moving boundary surface is not supported
  if (std::abs(surf1.evaluate({0, 0, 0}, 0.0)) > FP_COINCIDENT) {
    throw std::invalid_argument(fmt::format(
      "Rotational periodic BCs are only "
  ...
  if (std::abs(surf2.evaluate({0, 0, 0}, 0.0)) > FP_COINCIDENT) {
    throw std::invalid_argument(fmt::format(
      "Rotational periodic BCs are only "
      "supported for rotations about the origin, but surface {} does not "
      "intersect the origin.",
